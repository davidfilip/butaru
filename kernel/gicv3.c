...
static inline uint32 icc_igrpen1_el1() {
  uint32 x;
  asm volatile("mrs %0, S3_0_C12_C12_7" : "=r"(x));
  return x;
}

static inline void w_icc_igrpen1_el1(uint32 x) {
  asm volatile("msr S3_0_C12_C12_7, %0" : : "r"(x));
}

static inline uint32 icc_pmr_el1() {
  uint32 x;
  asm volatile("mrs %0, S3_0_C4_C6_0" : "=r"(x));
  return x;
}

static inline void w_icc_pmr_el1(uint32 x) {
  asm volatile("msr S3_0_C4_C6_0, %0" : : "r"(x));
}

static inline uint32 icc_iar1_el1() {
  uint32 x;
  asm volatile("mrs %0, S3_0_C12_C12_0" : "=r"(x));
  return x;
}

static inline void w_icc_eoir1_el1(uint32 x) {
  asm volatile("msr S3_0_C12_C12_1, %0" : : "r"(x));
}

static inline uint32 icc_sre_el1() {
  uint32 x;
  asm volatile("mrs %0, S3_0_C12_C12_5" : "=r"(x));
  return x;
}

static inline void w_icc_sre_el1(uint32 x) {
  asm volatile("msr S3_0_C12_C12_5, %0" : : "r"(x));
}

static void gicd_write(uint32 off, uint32 val) {
  *(volatile uint32 *)(gicv3.gicd + off) = val;
}

static uint32 gicd_read(uint32 off) {
  return *(volatile uint32 *)(gicv3.gicd + off);
}

static void gicr_write(uint32 cpuid, uint32 off, uint32 val) {
  *(volatile uint32 *)(gicv3.rdist_addrs[cpuid] + off) = val;
}

static uint32 gicr_read(uint32 cpuid, uint32 off) {
...
static void  gicrinit(uint32 cpuid) {
...
static void gic_set_target(uint32 intid, uint32 cpuid) {
...
static void gicr_enable_int(uint32 cpuid, uint32 intid) {
  uint32 is = gicr_read(cpuid, R_ISENABLER0);
  is |= 1 << (intid % 32);
  gicr_write(cpuid, R_ISENABLER0, is);
}

static void gicr_clear_pending(uint32 cpuid, uint32 intid) {
  uint32 ic = gicr_read(cpuid, R_ICPENDR0);
  ic |= 1 << (intid % 32);
  gicr_write(cpuid, R_ICPENDR0, ic);
}

static void gicr_set_prio0(uint32 cpuid, uint32 intid) {
